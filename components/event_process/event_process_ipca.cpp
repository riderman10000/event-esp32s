#include "event_process_ipca.hpp"

IncrementalPCA::IncrementalPCA(int components){
    n_components = components;
    n_samples_seen = 0;
}

// Incrementally update the covariance matrix
void IncrementalPCA::partial_fit(const cv::Mat& chunk_data) {
    int n_samples = chunk_data.rows;
    int n_features = chunk_data.cols;

    // Initialize the mean and covariance matrix if it's the first batch
    if (n_samples_seen == 0) {
        mean = cv::Mat::zeros(1, n_features, CV_64F);
        cov_matrix = cv::Mat::zeros(n_features, n_features, CV_64F);
    }

    // Compute the mean for the current chunk
    cv::Mat chunk_mean;
    cv::reduce(chunk_data, chunk_mean, 0, cv::REDUCE_AVG, CV_64F);  // Calculate mean along rows

    // Center the chunk data
    cv::Mat centered_chunk;
    cv::subtract(chunk_data, chunk_mean, centered_chunk);

    // Update the covariance matrix incrementally
    cov_matrix += centered_chunk.t() * centered_chunk;

    // Update the running mean
    mean = (mean * n_samples_seen + chunk_mean * n_samples) / (n_samples_seen + n_samples);

    // Update the total number of samples processed
    n_samples_seen += n_samples;
}

// Transform the data using the principal components
cv::Mat IncrementalPCA::transform(const cv::Mat& data) {
    // Perform eigen decomposition of the covariance matrix
    cv::Mat eigenvalues, eigenvectors;
    cv::eigen(cov_matrix, eigenvalues, eigenvectors);

    // Select the top 'n_components' principal components
    cv::Mat principal_components = eigenvectors.rowRange(eigenvectors.rows - n_components, eigenvectors.rows).clone();

    // Project the data onto the principal components
    cv::Mat centered_data;
    cv::subtract(data, mean, centered_data);
    cv::Mat reduced_data = centered_data * principal_components.t();

    return reduced_data;
}